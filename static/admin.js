...
async function load(){
 ...
 $('#topups').innerHTML=table(['ID','Khách','Gói','Tiền','Credits','Trạng thái',''],t.map(x=>[
 ...
 $('#users').innerHTML=table(['ID','Email','Tên','Credits','Role',''],u.map(x=>[
   x.id,x.email,x.name,x.credits,x.role,`<button class="mini-btn" onclick="addCredits(${x.id},'${x.email}')">± Credits</button>`
 ]));
 ...
 $('#affiliateWithdrawals').innerHTML=table(['ID','Khách','Credits','VND','Method','Account','Status',''],aw.map(x=>[
